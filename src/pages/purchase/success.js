...
export default function PurchaseSuccess() {
  ...
  useEffect(() => {
    async function loadSingle() {
      ...
      setPaidCents(Math.max(0, Math.round(qsPaid || (lineTotal - Math.max(0, Math.round(qsWallet))))));
      setLoading(false);
    }

    async function loadBasket() {
      let receipt = null;
      try { const raw = sessionStorage.getItem("lastCheckout"); if (raw) receipt = JSON.parse(raw); } catch {}
    ...
    setLoading(true);
    if (mode === "basket") loadBasket(); else loadSingle();
  ...
  const derivedSubtotalCents = useMemo(() => {
    ...
    return items.reduce((s, it) => s + Number(it.unit_price_cents || 0) * Number(it.qty || 0), 0);
  }, [items, subtotalCents]);
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        <div className="bg-white rounded-xl shadow p-6">
          ...
          {loading ? (
            ...
            <>
              <p className="mt-4 text-gray-700">Your order was completed, but we couldn’t load the item details.</p>
            ...
            <>
              <div className="mt-6 space-y-5">
                {items.map((it, idx) => {
                  ...
                  const unit = Number(it.unit_price_cents || 0) > 0 ? it.unit_price_cents / 100 : Number(h?.share_price || 0);
                  const qty = Number(it.qty || 0);
                  const lineTotal = unit * qty;

                  return (
                    <div key={`${it.horse_id}-${idx}`} className="flex gap-5">
                      <img src={h.photo_url || "https://placehold.co/320x200?text=Horse"} alt={h.name || "Horse"} className="w-44 h-28 object-cover rounded" />
                      <div>
                        <h2 className="text-xl font-semibold">{h.name || "Horse"}</h2>
                        <p className="text-sm text-gray-600">{(h.specialty || "—")} • Trainer: {(h.trainer || "—")}</p>
                        <div className="mt-3 text-sm">
                          <div>Quantity: <strong>{qty}</strong></div>
                          <div>Price per share: <strong>{fmtGBP(unit)}</strong></div>
                          <div className="mt-1">Total: <strong>{fmtGBP(lineTotal)}</strong></div>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="mt-6 rounded-lg border p-4 bg-gray-50">
                <div className="flex justify-between text-sm"><span>Subtotal</span><span>{fmtGBP(derivedSubtotalCents / 100)}</span></div>
                <div className="flex justify-between text-sm mt-1"><span>Wallet used</span><span className="text-emerald-700">−{fmtGBP(walletUsedCents / 100)}</span></div>
                <div className="flex justify-between text-base font-semibold mt-2"><span>Amount paid</span><span>{fmtGBP((paidCents || Math.max(0, derivedSubtotalCents - walletUsedCents)) / 100)}</span></div>
              </div>
              ...
              <p className="mt-6 text-xs text-gray-500">
                You’ll receive trainer updates and can enter ballots for owners’ badges and stable visits from your My Paddock dashboard.
              </p>
            </>
          )}
        </div>
